...
def_EHelper(add) { rtl_add(s, ddest, dsrc1, id_src2->preg); }
def_EHelper(addi) { rtl_addi(s, ddest, dsrc1, id_src2->imm); }
def_EHelper(sub) { rtl_sub(s, ddest, dsrc1, id_src2->preg); }
def_EHelper(xor) { rtl_xor(s, ddest, dsrc1, id_src2->preg); }
def_EHelper(xori) { rtl_xori(s, ddest, dsrc1, id_src2->imm); }
def_EHelper(or) { rtl_or(s, ddest, dsrc1, id_src2->preg); }
def_EHelper(ori) { rtl_ori(s, ddest, dsrc1, id_src2->imm); }
def_EHelper(and) { rtl_and(s, ddest, dsrc1, id_src2->preg); }
def_EHelper(andi) { rtl_andi(s, ddest, dsrc1, id_src2->imm); }
def_EHelper(sll) { rtl_sll(s, ddest, dsrc1, id_src2->preg); }
def_EHelper(slli) {
  // Log("slli" FMT_WORD ", %d", *dsrc1, id_src2->imm);
  rtl_slli(s, ddest, dsrc1, id_src2->imm);
  // Log("    -> " FMT_WORD, *ddest);
}
def_EHelper(srl) { rtl_srl(s, ddest, dsrc1, id_src2->preg); }
def_EHelper(srli) { rtl_srli(s, ddest, dsrc1, id_src2->imm); }
def_EHelper(sra) { rtl_sra(s, ddest, dsrc1, id_src2->preg); }
def_EHelper(srai) { rtl_srai(s, ddest, dsrc1, id_src2->imm); }
def_EHelper(slt) {
  rtl_li(s, ddest, (sword_t)*dsrc1 < (sword_t)*id_src2->preg ? 1 : 0);
}
def_EHelper(slti) {
  rtl_li(s, ddest, (sword_t)*dsrc1 < (sword_t)id_src2->imm ? 1 : 0);
}
def_EHelper(sltu) { rtl_li(s, ddest, *dsrc1 < *id_src2->preg ? 1 : 0); }
